...
CREATE VIEW TotalProductPrice AS
SELECT P_CODE, P_DESCRIPT, P_ONHAND, P_PRICE, 
(P_ONHAND * P_PRICE) AS TOTAL_PRICE, V_CODE
FROM Products
WHERE V_CODE IN (21344, 23119, 24288);

SELECT * FROM TotalProductPrice;
----------------------------------------
-- SP 4

DELIMITER //
CREATE PROCEDURE UpdateVendorName(IN old_name VARCHAR(30))
BEGIN 
...
DELIMITER //
CREATE FUNCTION ProductDetails(v_code INT, v_state VARCHAR(3))
RETURNS TEXT
DETERMINISTIC
BEGIN
    ...
    WHERE vendors.V_CODE = v_code AND vendors.V_STATE = v_state;

    RETURN product_info;
END //
DELIMITER ;

SELECT ProductDetails(21344, 'KY');
